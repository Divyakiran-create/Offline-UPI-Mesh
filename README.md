...
A Spring Boot backend that demonstrates **offline UPI payments routed through a Bluetooth-style mesh network**. You're in a basement with zero connectivity. You send your friend ₹500. Your phone encrypts the payment, broadcasts it to nearby phones, and the packet hops device-to-device until some phone walks outside, gets 4G, and uploads it to this backend. The backend decrypts, deduplicates, and settles.
...
3. **The backend rejects a tampered or replayed packet** before it touches the ledger.
...
The first run downloads Maven (~10 MB) and all dependencies (~80 MB), so give it a couple of minutes. Subsequent runs start in a few seconds.
...
- It encrypts that with the server's RSA public key (using hybrid encryption, described below).
...
After 1 round, every device holds the packet. After 2 rounds, every device still holds it, just with a lower TTL.

In the real system, this happens as people walk past each other in the basement.
...
The **Account Balances** table shows the money moved. The **Transaction Ledger** shows a new row.
...
To see idempotency in action, modify `MeshSimulatorService.java` to seed multiple bridge devices, or just:

1. Click "Inject" once.
2. Click "Gossip" twice.
3. Click "Flush Bridges." Only `phone-bridge` is a bridge in the default seed, so only one upload happens.

To exercise the *concurrent duplicate* case, run the test:
```cmd
...
**Why GCM?** It's authenticated encryption. If an intermediate flips one bit anywhere in the ciphertext, decryption throws an exception because the GCM tag won't verify. An attacker cannot trick the server into processing tampered data.

This is the same scheme TLS uses. See `HybridCryptoService.java`.

### Problem 2: The duplicate-storm

Three bridge nodes hold the same packet. They all walk outside at the same instant. They all POST to `/api/bridge/ingest` within milliseconds of each other. If you process all three without deduplication, the sender is debited ₹1500 instead of ₹500.

**Solution: Atomic compare-and-set on the ciphertext hash.**

The first thing the server does when it receives a packet is compute `SHA-256(ciphertext)` and try to "claim" that hash:
...
`ConcurrentHashMap.putIfAbsent` is atomic. Even if 100 threads call it at the exact same nanosecond, exactly one returns `null` (the first claimer) and the rest return the existing entry. Only the first claimer proceeds to decrypt and settle. The pipeline short-circuits the rest as `DUPLICATE_DROPPED`.

**Why hash the ciphertext, not the packetId or the cleartext?**
- A malicious intermediate can rewrite `packetId`, so two copies of the same payment could end up with different packetIds. Bad key.
- The cleartext requires decryption first. We want to dedupe *before* spending CPU on RSA.
- GCM authenticates the ciphertext, so decryption catches any tampering. Two legitimate deliveries of the same payment produce byte-identical ciphertexts, because AES is deterministic for a given key, IV, and plaintext, and the same packet means the same key, IV, and plaintext.

In production this `ConcurrentHashMap` becomes Redis: `SET key NX EX 86400`. Same semantics, distributed across replicas.

There's also a defense-in-depth fallback: `transactions.packet_hash` has a unique index. If the cache layer fails and two settlements try to write the same hash, the database rejects the second one.
...
The cryptography and idempotency code is production-shaped already. The infrastructure around it changes.

---

## Honest limitations of the concept

This design has real limits, worth stating plainly for anyone reviewing the project. These aren't implementation bugs; they're inherent to having no internet anywhere in the chain:

1. **The receiver has no way to verify the sender has the funds.** When sender hands receiver a phone showing "₹500 sent," it's an IOU, not a settled payment. If the sender's account is empty when the packet reaches the backend, the settlement will be `REJECTED` and the receiver is out ₹500 with no recourse. *This is why real offline UPI (UPI Lite) uses a pre-funded hardware-backed wallet*: it gives cryptographic proof of available funds offline.
...
3. **Bluetooth in real life is hard.** Android has throttled background BLE since Android 8. iOS locks down peripheral mode. Getting two strangers' phones to form a GATT connection while neither app is open takes real engineering effort and battery. This demo skips that problem by simulating the mesh.
4. **Privacy / liability.** A stranger carries your encrypted transaction packet on their phone. They can't read it, but its existence is metadata. In a real deployment, that raises questions about regulatory disclosure and what happens if a device is seized.

For a college / portfolio project: name the concept as **"mesh-routed deferred settlement"** rather than "real-time offline UPI," and you'll have a much stronger pitch. The cryptography and idempotency work here is real engineering and worth showing off.
...
**Tests fail intermittently**: The concurrency test is timing-sensitive. If it flakes, run it 3x; if it consistently fails on your hardware, file the actual failure output.